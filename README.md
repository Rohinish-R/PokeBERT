We recommend that you put all models in the same directory as RUNME.py, there will be 4 files to store regarding models:  
   1) best.pt  
   2) electra  
   3) w2v.model  
   4) embeddings  
     
Change IMAGE_LOCATION to the directory with the test images
Place the prompt data in test_prompts_orders.py, in the format given . The testing data we used is already provided in it.
Run RUNME.py once the dependencies are satisfied.
